...
int main(int argc, char**argv){
   ...
   while (dt > MAX_TEMP_ERROR && iteration <= max_iterations){
     ...
     for (i = 1; i<= ROWS; i++){
       for (j = 1; j<= COLS; j++){
         double delta = fabs(temperature[i][j]-temperature_last[i][j]);
	 dt = delta > dt ? delta:dt;
   ...
   printf("Total time was %d %f seconds.\n", (int)elapsed_time.tv_sec, ((float)elapsed_time.tv_sec + ((float)elapsed_time.tv_usec/1000000.0f)));
   exit(0);
}
